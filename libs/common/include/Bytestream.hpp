...
class CACAO_API bytestreambuf : public std::streambuf {
  public:
	 ...
	 *
	 * @param vec The vector to use as a buffer
	 */
	bytestreambuf(std::vector<char>& vec)
	  : buffer(vec) {
...
class CACAO_API ubytestreambuf : public std::streambuf {
  public:
	/**
	 * @brief Create a bytestreambuf from a vector of data
	 *
	 * @param vec The vector to use as a buffer
	 */
	ubytestreambuf(std::vector<unsigned char>& vec)
	  : buffer(vec) {
		setg(data(), data(), data() + buffer.size());
		setp(data(), data() + buffer.size());
	}
...
class CACAO_API ibytestream : public std::istream {
  public:
	ibytestream(std::vector<char>& data)
	  : std::istream(bufInit(data)), ubuf(nullptr), useUnsigned(false) {
		rdbuf(buf);
		init(buf);
	}

	ibytestream(std::vector<unsigned char>& data)
	  : std::istream(ubufInit(data)), buf(nullptr), useUnsigned(false) {
		rdbuf(ubuf);
		init(ubuf);
	}
  ...
  private:
	...
	ubytestreambuf* ubufInit(std::vector<unsigned char>& data) {
		ubuf = new ubytestreambuf(data);
		return ubuf;
	}
};
...
class CACAO_API obytestream : public std::ostream {
  public:
	obytestream(std::vector<char>& data)
	  : std::ostream(bufInit(data)), ubuf(nullptr), useUnsigned(false) {
		rdbuf(buf);
		init(buf);
	}

	obytestream(std::vector<unsigned char>& data)
	  : std::ostream(ubufInit(data)), buf(nullptr), useUnsigned(false) {
		rdbuf(ubuf);
		init(ubuf);
	}
