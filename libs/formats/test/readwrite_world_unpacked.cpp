...
int main() {
	try {
		//Write it
		{
			...
			std::ofstream str("./out.ajw");
			enc.EncodeWorld(w, str);
			str.close();
		}

		//Read it
		{
			std::ifstream str("./out.ajw");
			libcacaoformats::UnpackedDecoder dec;
			libcacaoformats::World w = dec.DecodeWorld(str);
			str.close();
			...
			if(e.initialPos.x != 1.0f || e.initialPos.y != 0.0f || e.initialPos.z != -1.4f) throw std::runtime_error("Wrong initial actor position!");
			if(e.initialRot.x != 0.0f || e.initialRot.y != 0.0f || e.initialRot.z != 0.0f) throw std::runtime_error("Wrong initial actor rotation!");
			if(e.initialScale.x != 2.0f || e.initialScale.y != 7.0f || e.initialScale.z != 3.0f) throw std::runtime_error("Wrong initial actor scale!");
