...
## `libcacaoasset`
`libcacaoasset` is a library for decoding the new Cacao Engine file formats based on [Jaguar](https://github.com/RobotLeopard86/Jaguar) streams. It succeeds the old `libcacaoformats` library, which dealt with the old, custom formats.  

Unlike its predecessor, `libcacaoasset` works exclusively with the compiled forms of these assets, though it does allow for both decoding and encoding of these files. Direct transformation of human-editable versions into the compiled forms is not a feature provided by this library.