...
namespace Cacao {
	...
	void UIView::Bind(int slot) {
		CheckException(std::this_thread::get_id() == Engine::GetInstance()->GetThreadID(), Exception::GetExceptionCodeFromMeaning("BadThread"), "Cannot bind UI view in non-rendering thread!");
	...
	void UIView::Unbind() {
		CheckException(std::this_thread::get_id() == Engine::GetInstance()->GetThreadID(), Exception::GetExceptionCodeFromMeaning("BadThread"), "Cannot unbind UI view in non-rendering thread!");
