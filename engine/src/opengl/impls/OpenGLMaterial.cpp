...
namespace Cacao {
	void OpenGLMaterialImpl::Apply(CommandBuffer* cmd) {
		...
		//Bind shader
		OpenGLCommandBuffer* glcb = static_cast<OpenGLCommandBuffer*>(cmd);
		OpenGLShaderImpl& glShader = RES_IMPL(Shader, OpenGL, *shader);
		glShader.Bind(glcb, renderMode == libcacaoasset::Material::RenderMode::Transparent);
