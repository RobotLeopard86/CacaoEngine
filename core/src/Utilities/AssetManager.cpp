	...
	std::future<AssetHandle<Shader>> AssetManager::LoadShader(std::string definitionPath) {
		return Engine::GetInstance()->GetThreadPool()->enqueue([this, definitionPath]() {
			...
			for(auto node : dfNode["spec"]) {
				...
				switch(idx) {
					case 0://int
						inf.type = ShaderDataType::Int;
						break;
					case 1://uint
						inf.type = ShaderDataType::UInt;
						break;
					case 2://float
						inf.type = ShaderDataType::Float;
						break;
					case 3://image
						inf.type = ShaderDataType::SampledImage;
						break;
					default:
						inf.type = ShaderDataType::Unknown;
						break;
				}
